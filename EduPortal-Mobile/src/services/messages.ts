...
export const sendMessage = async (receiverId: number, subject: string, content: string): Promise<Message> => {
  const response = await apiFetch<Message>('/messages', {
    method: 'POST',
    body: { receiver_id: receiverId, subject, content },
  });
  return response;
};
...
export const getMessages = async (page = 1, limit = 20): Promise<MessagesResponse> => {
  const response = await apiFetch<MessagesResponse>(`/messages?page=${page}&limit=${limit}`);
  return response;
};

// Get sent messages
export const getSentMessages = async (page = 1, limit = 20): Promise<MessagesResponse> => {
  const response = await apiFetch<MessagesResponse>(`/messages/sent?page=${page}&limit=${limit}`);
  return response;
};

// Mark message as read
export const markMessageAsRead = async (messageId: number): Promise<Message> => {
  const response = await apiFetch<Message>(`/messages/${messageId}/read`, {
    method: 'PATCH',
  });
  return response;
};
...
export const getUnreadCount = async (): Promise<UnreadCountResponse> => {
  const response = await apiFetch<UnreadCountResponse>('/messages/unread/count');
  return response;
};
...
export const getUsersForMessaging = async (): Promise<{ id: number; name: string; email: string; role: string }[]> => {
  const response = await apiFetch<{ users: { id: number; name: string; email: string; role: string }[] }>('/admin/users');
