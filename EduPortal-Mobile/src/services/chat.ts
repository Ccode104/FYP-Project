...
export async function loadChatSession(sessionId: string): Promise<{ session: ChatData }> {
  return apiFetch(`/chatbot/chats/${sessionId}`);
}
...
export async function chatWithAI(message: string, courseId?: number): Promise<{ response: string }> {
  return apiFetch('/chatbot/chat', {
    method: 'POST',
