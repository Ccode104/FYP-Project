...
export async function enrollStudent(offeringId: number, studentId: number) {
  return apiFetch(`/courses/offerings/${offeringId}/enroll`, {
    method: 'POST',
...
export async function listMyOfferings() {
  return apiFetch(`/courses/mine/offerings`)
}

export async function listMyCourses() {
  const data = await apiFetch(`/courses/card-data`)
  // Transform the card data to course format
  return data.courses.map((offering: any) => ({
    id: offering.id,
...
export async function createOffering(payload: { course_id: number; term: string; section?: string; faculty_id: number; max_capacity?: number; start_date?: string; end_date?: string }) {
  return apiFetch(`/courses/${payload.course_id}/offerings`, { method: 'POST', body: payload })
}