...
export async function getUserTickets(params?: {
  ...
  return apiFetch(`/support/tickets${query ? `?${query}` : ''}`)
}
...
export async function getAllTickets(params?: {
  ...
  const query = queryParams.toString()
  return apiFetch(`/support/admin/tickets${query ? `?${query}` : ''}`)
}
...
export async function updateTicketStatus(id: number, data: {
  status?: SupportTicket['status']
  assigned_to?: number
  priority?: SupportTicket['priority']
}): Promise<{ ticket: SupportTicket; message: string }> {
...
export async function addTicketComment(id: number, data: {
  ...
  return apiFetch(`/support/tickets/${id}/comments`, {
    method: 'POST',
    body: data
  })
}
