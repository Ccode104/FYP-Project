...
// NOTE: Copied as-is from legacy `services/admin.ts` (types kept file-local).
...
export async function updateUser(
  id: number,
  patch: Partial<{ role: 'student' | 'faculty' | 'ta' | 'admin'; department_id: number | null; is_active: boolean; name: string }>,
) {
...
export async function updateCourse(
  id: number,
  patch: Partial<{ code: string; title: string; description: string; department_id: number; credits: number }>,
) {
...
export async function updateOffering(
  id: number,
  patch: Partial<{ course_id: number; term: string; section: string; faculty_id: number; max_capacity: number; start_date: string; end_date: string }>,
) {
