...
export default function StudentDashboard() {
  ...
  // Load available offerings when modal opens
  useEffect(() => {
  ...
  const loadAvailableOfferings = async () => {
    try {
      ...
      const data = await apiFetch<any[]>('/api/courses/available-offerings')
  ...
  return (
    <div className="container container-wide dashboard-page student-theme">
      ...
      <Modal open={enrOpen} onClose={() => setEnrOpen(false)} title={(user?.role === 'student') ? 'Enroll in Course' : 'Enroll Student'} actions={(
        <>
          <button className="btn" onClick={() => setEnrOpen(false)}>Cancel</button>
          <button className="btn btn-primary" onClick={enrollNow} disabled={!offId || loadingOfferings}>Enroll</button>
        </>
      )}>
        <div className="form">
          {user?.role === 'student' ? (
            <>
              <label className="field">
                ...
                {loadingOfferings ? (
                  ...
                  <div style={{ padding: '12px', textAlign: 'center', color: '#999' }}>No courses available to enroll</div>
                ) : (
                  <select 
                    className="input" 
                    value={offId} 
                  ...
                  >
                    ...
                    {availableOfferings.map((offering) => (
                      <option key={offering.id} value={offering.id}>
                        {offering.course_code || `Course ${offering.course_id}`} - {offering.course_title} ({offering.term} {offering.section ? `${offering.section}` : ''}) 
