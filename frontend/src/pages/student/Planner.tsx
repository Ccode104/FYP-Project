...
export default function Planner() {
  ...
  const loadPlanner = async () => {
    try {
      setLoading(true);
      const [taskData, prefData] = await Promise.all([
        fetchPlannerTasks(),
        fetchPlannerPreferences()
      ]);
      ...
      push({ kind: 'error', message: error?.message || 'Failed to load planner' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerate = async () => {
    try {
      ...
      push({ kind: 'error', message: error?.message || 'Failed to generate plan' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      push({ kind: 'error', message: error?.message || 'Failed to create task' });
    }
  };
  ...
  const updateStatus = async (task: PlannerTask, status: PlannerTask['status']) => {
    try {
      ...
      push({ kind: 'error', message: error?.message || 'Failed to update task' });
    }
  };
  ...
  const handleDelete = async (taskId: number) => {
    try {
      ...
      push({ kind: 'error', message: error?.message || 'Failed to delete task' });
    }
  };
  ...
  const handleDrop = async (targetId: number) => {
    ...
    try {
      await reorderPlannerTasks(
        reordered.map((task, index) => ({ id: task.id, order_index: index }))
      );
    } catch (error: unknown) {
      push({ kind: 'error', message: error?.message || 'Failed to reorder tasks' });
    }
  };
  ...
  const handlePreferenceUpdate = async (next: Partial<PlannerPreferences>) => {
    ...
    try {
      ...
      push({ kind: 'error', message: error?.message || 'Failed to update preferences' });
    }
  };
    ...
    <div className="container container-wide planner-page">
      <div className="planner-header">
        ...
        <div className="planner-actions">
          <button className="btn btn-secondary" onClick={() => setShowModal(true)}>Add Task</button>
          <button className="btn btn-primary" onClick={handleGenerate} disabled={loading}>Auto-Generate Plan</button>
        </div>
      </div>
            ...
            {filteredTasks.map(task => (
              ...
              >
                ...
                <div className="task-actions">
                  ...
                  <button className="btn btn-ghost" onClick={() => handleDelete(task.id)}>Remove</button>
                </div>
              </div>
            ))}
          </div>
